...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // from draining ElevenLabs credits while still serving public visitors.
    ...
    if (!resp.ok) {
      ...
      // Rate-limit, quota/auth failures, or upstream errors → tell client to fall back
      // to the browser SpeechSynthesis API so the CPR guide keeps talking.
      const isQuota = /quota_exceeded/i.test(errTxt);
